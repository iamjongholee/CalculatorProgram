...
public final class Stack<T>
{
	...
	{// 인덱스로 꺼내기.
		...
		int deeps = (this.size() - 1) / this.slotSize;// 전체 슬롯의 깊이.
		int floorDownCount = (deeps + 1) * this.slotSize - 1 - index;// top 부터 내려갈 카운트 개수.
		int inSlotIndex = (this.slotSize - (floorDownCount % this.slotSize) - 1);// 찾은 슬롯에서의 해당 멤버 위치.
		for(int i = floorDownCount / this.slotSize ; i > 0; --i)
		{// i는 찾아 내려갈 슬롯 깊이.
			taskSlot = taskSlot.getFrontSlot();// 인덱스에 해당하는 데이터를 가지고 있는 슬롯을 만날 때까지 내려감.
			}
